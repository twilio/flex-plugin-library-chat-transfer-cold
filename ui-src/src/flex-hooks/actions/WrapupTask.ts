...
import { getWorkerFriendlyName }  from '../../service/ChatTransferService';
import TaskService from '../../service/TaskRouterService';
import ProgrammableChatService from '../../service/ProgrammableChatService';
...
export const removeChannelSidAndLeaveChatForChatTransfer = async (flex: typeof Flex, manager: Flex.Manager) => {
  ...
  try{
    ...
    ErrorManager.createAndProcessError("Could not add 'beforeWrapupTask' listener", {
      type: FlexPluginErrorType.action,
      description: e instanceof Error ? `${e.message}` : "Could not add 'beforeWrapupTask' listener",
      context: "Plugin.Action.beforeWrapupTask",
      wrappedError: e
    });
  }
